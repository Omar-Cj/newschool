...
@section('content')
    <div class="page-content">
        ...
        <div class="card ot-card">
            <div class="card-body">
                <form action="{{ route('permission-features.update', $data['permission_feature']->id) }}" method="post" id="permission-feature-form-edit">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control ot-input @error('name') is-invalid @enderror" name="name"
                                id="name" type="text"
                                placeholder="{{ ___('common.Enter name') }}" value="{{ old('name', $data['permission_feature']->name) }}">
                            @error('name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="nice-select niceSelect bordered_style wide @error('feature_group_id') is-invalid @enderror"
                                ...
                                @foreach($data['feature_groups'] as $group)
                                    <option value="{{ $group->id }}" {{ old('feature_group_id', $data['permission_feature']->feature_group_id) == $group->id ? 'selected' : '' }}>
                                        {{ $group->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="nice-select niceSelect bordered_style wide @error('permission_id') is-invalid @enderror"
                                ...
                                @foreach($data['permissions'] as $permission)
                                    <option value="{{ $permission->id }}" {{ old('permission_id', $data['permission_feature']->permission_id) == $permission->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control ot-input @error('position') is-invalid @enderror" name="position"
                                ...
                                placeholder="0" value="{{ old('position', $data['permission_feature']->position) }}">
                            @error('position')
                                <div id="validationServer04Feedback" class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="is_premium" id="is_premium"
                                    value="1" {{ old('is_premium', $data['permission_feature']->is_premium) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="nice-select niceSelect bordered_style wide @error('status') is-invalid @enderror"
                                ...
                                <option value="1" {{ old('status', $data['permission_feature']->status) == 1 ? 'selected' : '' }}>{{ ___('mainapp_common.active') }}</option>
                                <option value="0" {{ old('status', $data['permission_feature']->status) == 0 ? 'selected' : '' }}>{{ ___('mainapp_common.inactive') }}</option>
                            </select>
                            @error('status')
                                <div id="validationServer04Feedback" class="invalid-feedback">
                                    {{ $message }}
