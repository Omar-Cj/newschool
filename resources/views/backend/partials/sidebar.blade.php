<aside class="sidebar" id="sidebar">
    ...
    @php
        // LOG POINT 15: Sidebar rendering context
        if (Auth::check()) {
            Log::channel('feature_access')->debug('Sidebar rendering', [
                'user_id' => Auth::id(),
                'user_role_id' => Auth::user()->role_id,
                'user_school_id' => Auth::user()->school_id,
                'school_package_id' => Auth::user()->school?->package_id,
            ...
            <ul class="sidebar-dropdown-menu">
                ...
                @if (
                    (hasPermission('student_read') ||
                        ...
                        hasAnyFeature(['student', 'admission', 'parent', 'promote_students', 'disabled_students', 'student_category', 'student_reports_read']))
                ...
                @if (
                    (hasPermission('classes_read') ||
                        ...
                        hasAnyFeature(['classes', 'section', 'shift', 'class_setup', 'subject', 'subject_assign', 'time_schedule', 'class_room']))
                ...
                {{-- start routines --}}
                @if ((hasPermission('class_routine_read') || hasPermission('exam_routine_read')) && hasAnyFeature(['class_routine', 'exam_routine']))
                ...
                @if (
                    (hasPermission('fees_group_read') ||
                        ...
                        hasAnyFeature(['fees_type', 'fees_collect', 'fees_generation', 'fees_group', 'fees_master', 'fees_assign', 'parent_deposit', 'parent_statement', 'discount_setup']))
                    <li class="sidebar-menu-item {{ set_menu(['fees*']) }}">
                        ...
                        <ul class="child-menu-list">
                            @if (auth()->user()->school_id ? (hasFeature('fees_group') && hasPermission('fees_group_read')) : hasPermission('fees_group_read'))
                            ...
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('fees_master') && hasPermission('fees_master_read')) : hasPermission('fees_master_read'))
                                <li class="sidebar-menu-item {{ set_menu(['fees-master*']) }}">
                                    <a href="{{ route('fees-master.index') }}">{{ ___('settings.master') }}</a>
                                </li>
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('fees_assign') && hasPermission('fees_assign_read')) : hasPermission('fees_assign_read'))
                            ...
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('fees_generation') && hasPermission('fees_generate_read')) : hasPermission('fees_generate_read'))
                ...
                @if (
                    ...
                    <li
                        ...
                        <ul class="child-menu-list">
                            ...
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('exam_entry') && hasPermission('exam_entry_read')) : hasPermission('exam_entry_read'))
                ...
                @if (
                    (hasPermission('question_group_read') ||
                        ...
                        hasAnyFeature(['online_exam', 'online_exam_type', 'question_bank', 'question_group']))
                    <li
                ...
                @if (
                    (hasPermission('book_category_read') ||
                        ...
                        hasAnyFeature(['book', 'book_category', 'member', 'member_category', 'issue_book']))
                ...
                @if (
                    (hasPermission('account_head_read') || hasPermission('income_read') || hasPermission('expense_category_read') || hasPermission('expense_read') || hasPermission('journal_read') || hasPermission('cash_transfer_read')) &&
                        hasAnyFeature(['account', 'account_head', 'income', 'expense', 'expense_category', 'journal', 'cash_transfer']))
                    <li
                        ...
                        <ul class="child-menu-list">
                            ...
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('expense_category') && hasPermission('expense_category_read')) : hasPermission('expense_category_read'))
                            ...
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('cash_transfer') && hasPermission('cash_transfer_read')) : hasPermission('cash_transfer_read'))
                ...
                @if (
                    (hasPermission('report_marksheet_read') ||
                        ...
                        hasAnyFeature(['report_center', 'marksheet', 'merit_list', 'progress_card', 'due_fees', 'fees_collection', 'account', 'class_routine', 'exam_routine', 'attendance_report', 'student_reports_read']))
                    <li class="sidebar-menu-item {{ set_menu(['report-*']) }}">
                        ...
                        <ul class="child-menu-list">
                            ...
                            @endif
                            @if (auth()->user()->school_id ? (hasFeature('report_center') && hasPermission('report_center_read')) : hasPermission('report_center_read'))
                ...
                @if (
                    (hasPermission('page_sections_read') ||
                        ...
                        hasAnyFeature(['sections', 'slider', 'about', 'counter', 'contact_info', 'dep_contact', 'news', 'event']))
                    <li
                ...
                <!-- Gallery start -->
                @if ((hasPermission('gallery_category_read') || hasPermission('gallery_read')) && hasAnyFeature(['gallery', 'gallery_category']))
                ...
                @if (
                    (hasPermission('general_settings_read') ||
                        ...
                        hasAnyFeature(['general_settings', 'storage_settings', 'task_schedules', 'software_update', 'recaptcha_settings', 'email_settings', 'sms_settings', 'payment_gateway_settings', 'genders', 'religions', 'blood_groups', 'sessions', 'language', 'tax_setup']))
                    ...
                    <li class="sidebar-menu-item {{ set_menu(['setting*', 'genders*', 'religions*']) }}">
                        ...
                        <ul class="child-menu-list">
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('notification_settings') && hasPermission('notification_settings_read')) : hasPermission('notification_settings_read'))
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('storage_settings') && hasPermission('storage_settings_read')) : hasPermission('storage_settings_read'))
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('task_schedules') && hasPermission('task_schedules_read')) : hasPermission('task_schedules_read'))
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('software_update') && hasPermission('software_update_read')) : hasPermission('software_update_read'))
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('recaptcha_settings') && hasPermission('recaptcha_settings_read')) : hasPermission('recaptcha_settings_read'))
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('sms_settings') && hasPermission('sms_settings_read')) : hasPermission('sms_settings_read'))
                                <li class="sidebar-menu-item {{ set_menu(['settings.recaptcha-setting']) }}">
                                    <a
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('payment_gateway_settings') && hasPermission('payment_gateway_settings_read')) : hasPermission('payment_gateway_settings_read'))
                            ...
                            @if (auth()->user()->school_id ? (hasFeature('email_settings') && hasPermission('email_settings_read')) : hasPermission('email_settings_read'))
