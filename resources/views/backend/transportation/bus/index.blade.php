            ...
            <div class="card ot-card mb-24 position-relative z_1">
                <div class="card-header d-flex align-items-center gap-4 flex-wrap">
                    ...
                    <div class="card_header_right d-flex align-items-center gap-3 flex-fill justify-content-end flex-wrap">
                        ...
                        <!-- Students Assignment Filter -->
                        <div class="single_large_selectBox">
                            <select id="studentsFilter" class="form-select nice-select niceSelect bordered_style wide" name="has_students">
                                <option value="">{{ ___('transportation.all_buses') }}</option>
                                <option value="yes">{{ ___('transportation.with_students') }}</option>
                                <option value="no">{{ ___('transportation.without_students') }}</option>
                            </select>
                        </div>

                        <!-- Keyword Search -->
                        <div class="single_large_selectBox">
                            <input class="form-control ot-input" id="keywordFilter" placeholder="{{ ___('transportation.keyword_search') }}">
                        </div>
        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if (hasPermission('bus_create'))
                        <a href="{{ route('bus.create') }}" class="btn btn-lg ot-btn-primary">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="busTable" class="table table-bordered class-table">
                            <thead class="thead">
                                <tr>
                                    <th class="serial">{{ ___('common.sr_no') }}</th>
                                    <th class="purchase">{{ ___('transportation.area_name') }}</th>
                                    <th class="purchase">{{ ___('transportation.bus_number') }}</th>
                                    <th class="purchase">{{ ___('transportation.capacity') }}</th>
                                    <th class="purchase">{{ ___('transportation.driver_name') }}</th>
                                    <th class="purchase">{{ ___('transportation.license_plate') }}</th>
                                    <th class="purchase">{{ ___('common.status') }}</th>
                                    <th class="purchase">{{ ___('transportation.students_count') }}</th>
...
@push('script')
    ...
    <style>
        ...
        .table-content.table-basic #busTable {
            font-size: 14px;
        }

        .table-content.table-basic #busTable thead th {
        ...
        .table-content.table-basic #busTable tbody td {
            padding: 10px 8px;
            font-size: 13px;
        ...
        .table-content.table-basic #busTable tbody tr:nth-of-type(odd) {
        ...
        .table-content.table-basic #busTable thead th.sorting,
        .table-content.table-basic #busTable thead th.sorting_asc,
        .table-content.table-basic #busTable thead th.sorting_desc {
            cursor: pointer;
            position: relative;
        }

        .table-content.table-basic #busTable thead th.sorting:after,
        .table-content.table-basic #busTable thead th.sorting_asc:after,
        .table-content.table-basic #busTable thead th.sorting_desc:after {
        ...
        /* Small badge variant for students count - more compact display */
        .badge-sm {
            font-size: 10px;
            padding: 2px 6px;
            font-weight: 500;
        ...
        /* Enhanced contrast for capacity indicator badges */
        .badge-info {
    ...
    <script>
        let busTable;
        let ajaxUrl = '{{ route("bus.ajaxData") }}';
            ...
            busTable = $('#busTable').DataTable({
                ...
                ajax: {
                    ...
                    data: function(d) {
                        ...
                        d.has_students = $('#studentsFilter').val();
                        d.keyword = $('#keywordFilter').val();
                    },
                    error: function(xhr, error, thrown) {
                        console.error('DataTables AJAX error:', error, thrown);
                        showErrorMessage('Failed to load bus data. Please refresh the page.');
                    }
                },
                ...
                columns: [
                    ...
                    { data: 1, name: 'area_name', orderable: true, searchable: true },
                    { data: 2, name: 'bus_number', orderable: true, searchable: true },
                    { data: 3, name: 'capacity', orderable: true, searchable: false, width: '8%' },
                    { data: 4, name: 'driver_name', orderable: true, searchable: true },
                    { data: 5, name: 'license_plate', orderable: true, searchable: true },
                    { data: 6, name: 'status', orderable: true, searchable: false, width: '10%' },
                    { data: 7, name: 'students_count', orderable: true, searchable: false, width: '10%' },
                    @if (hasPermission('bus_update') || hasPermission('bus_delete'))
                    { data: 8, name: 'actions', orderable: false, searchable: false, width: '10%' }
                    @endif
                ],
                order: [[1, 'asc']], // Default sort by area name
                pageLength: 10,
        ...
        function setupFilterHandlers() {
            ...
            // Students filter change handler
            $('#studentsFilter').on('change', function() {
                busTable.ajax.reload();
            });
